...
const logs = [];
const tafLog = (msg) => { if (!logs.includes(msg)) { console.log(`[TAF]`, msg); logs.push(msg); } }
...
// resize image to 8x8
function imageToCompressData(image, imgWidth = 8) {
...
async function downloadImage(url, subUrl) {
    // get filename
    ...
    let image = await canvas.loadImage(filepath).catch((e) => {
        console.log(`[TAF] download image error: ${url}`);
    ...
    if (!image) {
        ...
        if (subUrl != null) {
            console.log(`[TAF] try sub url: ${subUrl}`);
...
async function imageComparison(image1) {

    // get all blacklist images
    for (let file2 of imagesList) {
        let image2 = await canvas.loadImage(file2).catch((e) => { console.log(`[TAF]`, file2, e.message); return null; });
        if (!image2) { continue; }
...
class AntiFilterCore {
    ...
    async getImageComparison(username, tID, embedImage) {
        ...
        if (tweetStatu == 1) {
            ...
            console.log(`[TAF] getImageComparison timeout, tID: ${tID}`);
            return false;
        ...
        let imageInBlackList = await imageComparison(image.canvas).catch(e => console.log(`[TAF]`, e.message));
        if (imageInBlackList) {
            // image in blacklist
    ...
    async logToDiscord(message, cID = CODE_CHANNEL) {
        ...
        if (this.channels.has(cID)) {
            ...
            channel.send(payload).catch(e => console.log(`[TAF]`, e.message));
    ...
    async backupImageToDiscord(imageFile) {
        ...
        await channel.send({ files }).catch(e => console.log(`[TAF]`, e.message));
    ...
    uploadBlacklist(delay = 120 * 1000) {
        ...
        this.uploadTimout = setTimeout(async () => { await this._uploadBlacklist(); this.uploadTimout = null }, delay);
    }
    async _uploadBlacklist() {
        ...
        if (files.length > 0) {
            await msg.edit({ files }).catch(e => console.log(`[TAF]`, e.message));
...
class SpamUserList {
    ...
    async addUser(username, uID, addTime) {
        ...
        if (!/^\d+$/.test(uID)) {
            ...
            if (fs.existsSync("./.env")) { console.log(`[TAF] ${username} uID: <${uID}>`); }
            ...
            if (!/^\d+$/.test(_uID)) {
                let log = `[TAF] Get user ${username} identifier fail! uID: ${uID}`;
                if (fs.existsSync("./.env")) { console.log(log); }
                return log;
            }
        ...
        if (this.userList.has(uID)) {
            let log = `[TAF] Add user ${username} fail, found exist uID!`;
            console.log(log);
...
async function messageExecute(message) {
    ...
    // console.log(`[TAF] username: <${username}>, uID: <${uID}>`)
        ...
        if (command == 'blimg') {
            ...
            for (let file of imagesList) {
                ...
                if (result.length + line.length > 1980) {
                    await channel.send({ content: `\`\`\`${result}\`\`\`` }).catch(e => console.log(`[TAF]`, e.message));
                    result = line;
                    continue;
                }
...
class Twitter {
    ...
    async getUserID(username) {
        ...
        if (!uID) {
            // twitter API v2
            const user = await this.client.v2.userByUsername(username).catch(e => console.log(`[TAF]`, e.message));
    ...
    async getUsername(uID) {
        ...
        if (!username) {
            // twitter API v2
            const user = await this.client.v2.user(uID).catch(e => console.log(`[TAF]`, e.message));
...
// function generateTimeline(username){
