...
const reactionRole = async (reaction, user, pluginConfig, add) => {
    ...
    if (!guild.members.me.permissions.has(PermissionFlagsBits.ManageRoles)) {
        console.log('[Role] Missing Permissions: MANAGE_ROLES');
        return false;
    }
    ...
    for (const line of lines) {
        ...
        if (!role) {
            console.log(`[Role] Unknown role: <@&${roleID}>`)
            continue;
        }
    ...
    if (add) { await target.roles.add(emojiRole.role).catch(e => console.log(`[Role]`, e.message)); }
    else { await target.roles.remove(emojiRole.role).catch(e => console.log(`[Role]`, e.message)); }
