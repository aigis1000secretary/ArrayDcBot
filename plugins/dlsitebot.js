...
const getDLsiteHtml = async (index) => {
    ...
    if (!ajax) { return null; }
    let siteID = ajax.body[index].site_id;

    let res, url;
    try {
        // request
        url = `https://${ajax.req.host}/${siteID}/announce/=/product_id/${index}.html`;
        res = await get({ url });
        if (res?.statusCode == 200 && !!res?.body) { return res; }

        // retry for work type
        url = `https://${ajax.req.host}/${siteID}/work/=/product_id/${index}.html`;
        res = await get({ url });
        if (res?.statusCode == 200 && !!res?.body) { return res; }
    } catch (e) { }
